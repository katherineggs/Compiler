...
import abstracts.Block as stop
...
class ParseDFA:
    ...
    gramaticaFinal = [ {'<program>': ['class','Program', '{', '}']} ]
    dfaFinal = {
    ...
    gramTot = [ {'block': ['{', 'statement_list', '}']},  # 1
        ...
        {'statement': ['for', 'id', '=','expr', ',', 'expr', 'block']},  # 9
        {'statement': ['return', ';']},  # 10
        ...
        {'method_call': ['callout', '(', 'string_literal', ',', 'callout_arg_list', ')']},  # 29
    ...
    def parse(self, program, nodoInicial, tipoDfa, debug, listaErrores):
        listaNodos = nodoInicial.listaNodos

        if tipoDfa == 'field_decl_list' :
            nuevosNodos = []
            contNodos = 0
            contsIndexs = []
            for nodo in listaNodos:
                if(nodo.objeto.tipoS.nombre == ';'):
                    contsIndexs.append(contNodos)
                contNodos += 1   

            fields = []
            for index in range(len(contsIndexs)):
                if(index == 0):
                    fields.append(listaNodos[0:contsIndexs[index]+1])
                else:
                    fields.append(listaNodos[contsIndexs[index-1]+1:contsIndexs[index]+1])
                    
            for fieldDec in fields:
                if len(fieldDec) < 3:
                    listaErrores.append("Parse error: Not enough tokens in field declaration at line " + str(fieldDec[0].objeto.linea))
                if len(fieldDec) == 3 :
                    if fieldDec[0].objeto.tipoS.name != 'type':
                        listaErrores.append("Parse error: Unexpected token "+ fieldDec[0].objeto.tipoS.name +" at line "+str(fieldDec[0].objeto.line))
                    if fieldDec[1].objeto.tipoS.name != 'id':
                        listaErrores.append("Parse error: Unexpected token "+ fieldDec[1].objeto.tipoS.name +" at line "+str(fieldDec[1].objeto.line))
                    if fieldDec[2].objeto.tipoS.name != ';':
                        listaErrores.append("Parse error: Unexpected token "+ fieldDec[2].objeto.tipoS.name +" at line "+str(fieldDec[2].objeto.line))
                    if (fieldDec[0].objeto.tipoS.name == 'type') and (fieldDec[1].objeto.tipoS.name == 'id') and (fieldDec[2].objeto.tipoS.name == ';'):
                        field_obj = field.FieldDecl()
                        # Nodo.Nodo(nodo, tipo, listaNodos)
                        nuevosNodos.append(Nodo.Nodo(field_obj, "fieldDec", [fieldDec[0], fieldDec[1], fieldDec[2]]))
                ...
                if len(fieldDec) > 3:
                    ...
                    if fieldDec[0].objeto.tipoS.name != 'type' :
                        listaErrores.append("Parse error: Unexpected token "+ fieldDec[0].objeto.tipoS.name +" at line "+str(fieldDec[0].objeto.line))
                        id_list_bool = False
                    for id_index in range(1, len(fieldDec)-1):
                        if(id_index%2!=0):
                            if(fieldDec[id_index].objeto.tipoS.name != 'id'):
                                listaErrores.append("Parse error: Unexpected token "+ fieldDec[id_index].objeto.tipoS.name +" at line "+str(fieldDec[id_index].objeto.line))
                                id_list_bool = False
                            else:
                                temp_ids.append(fieldDec[id_index])
                        else:
                            if(fieldDec[id_index].objeto.tipoS.name != ','):
                                listaErrores.append("Parse error: Unexpected token "+ fieldDec[id_index].objeto.tipoS.name +" at line "+str(fieldDec[id_index].objeto.line))
                                id_list_bool = False
                    if(id_list_bool):
                        for temp_id in temp_ids:
                            field_obj = field.FieldDecl()
                            nuevosNodos.append(Nodo.Nodo(field_obj, "fieldDec", [fieldDec[0], temp_id, fieldDec[len(fieldDec)-1]]))

            nodoInicial.listaNodos = nuevosNodos
            program.listaNodos[3] = nodoInicial

        if(debug):

            print("Error list", listaErrores)

    def parse_method(self, program, nodoInicial, tipoDfa, debug, listaErrores):
        nuevosNodos_method = []
        listaNodos = nodoInicial.listaNodos
        ...
        if(len(listaNodos)<6):
            ...
            if(listaNodos[0].objeto.tipoS.name != "type" and listaNodos[0].objeto.tipoS.name != "void"):
                listaErrores.append("Parse error: Unexpected token "+ listaNodos[0].objeto.tipoS.name +" at line "+str(listaNodos[0].objeto.line))
            if(listaNodos[1].objeto.tipoS.name != "id"):
                listaErrores.append("Parse error: Unexpected token "+ listaNodos[1].objeto.tipoS.name +" at line "+str(listaNodos[1].objeto.line))
            if(listaNodos[2].objeto.tipoS.name != "("):
                listaErrores.append("Parse error: Unexpected token "+ listaNodos[2].objeto.tipoS.name +" at line "+str(listaNodos[2].objeto.line))
            
            for contNodos in range(len(listaNodos)):
                if(contNodos<len(listaNodos)-3):
                    if(
                        (listaNodos[contNodos].objeto.tipoS.name == "type" or listaNodos[contNodos].objeto.tipoS.name == "void") and
                         listaNodos[contNodos+1].objeto.tipoS.name == "id" and
                         listaNodos[contNodos+2].objeto.tipoS.name == "(" ):

                        method_decl_obj = MethodDecl.MethodDecl()
                        method_decl_node = Nodo.Nodo(method_decl_obj, "method_decl", [listaNodos[contNodos], listaNodos[contNodos+1], listaNodos[contNodos+2]])
                        nuevosNodos_method.append(method_decl_node)

                        counter_1 = contNodos+3
                        child_var_decl_list=[]
                        tout_bien = True
                        while(listaNodos[counter_1].objeto.tipoS.name != ")" and counter_1<len(listaNodos)-3):
                            tout_bien = True
                            if(listaNodos[counter_1].objeto.tipoS.name == "type"):
                                if(listaNodos[counter_1+1].objeto.tipoS.name != "id" 
                                    or (listaNodos[counter_1+2].objeto.tipoS.name != "," and listaNodos[counter_1+2].objeto.tipoS.name != ")")):
                                    listaErrores.append("Parse error: Unexpected token "+ listaNodos[counter_1].objeto.tipoS.name +" at line "+str(listaNodos[counter_1].objeto.line))
                                    tout_bien = False
                                else:
                                    child_var_decl_list.append(listaNodos[counter_1])

                            elif(listaNodos[counter_1].objeto.tipoS.name == "id"):
                                if((listaNodos[counter_1+1].objeto.tipoS.name != "," and listaNodos[counter_1+1].objeto.tipoS.name != ")")
                                    or listaNodos[counter_1-1].objeto.tipoS.name != "type" ):
                                    listaErrores.append("Parse error: Unexpected token "+ listaNodos[counter_1].objeto.tipoS.name +" at line "+str(listaNodos[counter_1].objeto.line))
                                    tout_bien = False
                                else:
                                    child_var_decl_list.append(listaNodos[counter_1])

                            elif(listaNodos[counter_1].objeto.tipoS.name == ","):
                                if(listaNodos[counter_1+1].objeto.tipoS.name != "type"
                                    or listaNodos[counter_1-1].objeto.tipoS.name != "id" ):
                                    listaErrores.append("Parse error: Unexpected token "+ listaNodos[counter_1].objeto.tipoS.name +" at line "+str(listaNodos[counter_1].objeto.line))
                                    tout_bien = False
                                else:
                                    child_var_decl_list.append(listaNodos[counter_1])
                            ...
                            counter_1+=1
                        if(listaNodos[counter_1].objeto.tipoS.name != ")"):
                            listaErrores.append("Parse error: Missing closing ) in method declaration args")
                        var_decl_object = VarDeclList.VarDeclList()
                        var_decl_node = Nodo.Nodo(var_decl_object, "var_decl_list", [])
                        if(tout_bien):
                            var_decl_node.listaNodos = child_var_decl_list
                        method_decl_node.listaNodos.append(var_decl_node)
                        method_decl_node.listaNodos.append(listaNodos[counter_1])
                        ...
                        if(listaNodos[counter_1].objeto.tipoS.name == "{"):
                                ...
                                while not(
                                        (listaNodos[counter_1].objeto.tipoS.name == "type" or listaNodos[counter_1].objeto.tipoS.name == "void") and
                                        listaNodos[counter_1+1].objeto.tipoS.name == "id" and
                                        listaNodos[counter_1+2].objeto.tipoS.name == "("
                                    ) and counter_1<len(listaNodos)-2:

                                    block_children_list.append(listaNodos[counter_1])
                                    counter_1+=1
                                if(counter_1==len(listaNodos)-2):
                                    block_children_list.append(listaNodos[counter_1])

                                    block_children_list.append(listaNodos[counter_1+1])                              

                        block_object = Block.Block()
                        block_node = Nodo.Nodo(block_object, "block", block_children_list)
                        method_decl_node.listaNodos.append(block_node)

            nodoInicial.listaNodos=nuevosNodos_method
        if debug:
            print(listaErrores)

    def parse_block(self, program, nodoInicial, debug, listaErrores):
        initial_node = Nodo.Nodo("$", "$", [])
        states_stack = [0]
        nodes_stack = [initial_node]

        listaNodos_analize = nodoInicial.listaNodos
        
        len_list = len(listaNodos_analize)
        index = 0
        last_state = states_stack[-1]
        param_list = []
        current_node = listaNodos_analize[last_state]
        param_list = self.dfa_parse_1.get(last_state).get(current_node.type_node)
        if(param_list==None):
            listaErrores.append("Parse error: Missing opening { in line " + str(current_node.objeto.line))
        ...
        while(index<len_list and rip<15):
            if (param_list != None):
                if(param_list[0]=='shift'):
                    ...
                    index+=1
                    current_node = listaNodos_analize[index]
                    ...
                    if(param_list[1] == 2):  
                        ...
                        for node_verify in nodes_stack[::-1]:
                            if(node_verify.type_node == "statement"):
                                ...
                                listaErrores.append("Parsing error, unexpected production "+node_verify.type_node)
                                break

                        statement_list_node = Nodo.Nodo("statement_list", "statement_list", temp_statement_list[::-1])
                        node_block = Nodo.Nodo("block", "block", [open_bracket, statement_list_node, listaNodos_analize[index]])
                        ...
                        objeto = list(self.grammer_1[param_list[1]-1].keys())[0]
                        type_node = list(self.grammer_1[param_list[1]-1].keys())[0]
                        count = len(list(self.grammer_1[param_list[1]-1].values())[0])
                        child_listaNodos = nodes_stack[-count:]
                        nodes_stack = nodes_stack[:-count]
                        states_stack = states_stack[:-(count)]
                        new_node = Nodo.Nodo(objeto, type_node, child_listaNodos)
                        ...
                        if(param_list!=None and param_list[0]=='goto' and param_list[1]==32):
                            if(listaNodos_analize[index].type_node == "{" and nodes_stack[-1].type_node == "expr" and nodes_stack[-2].type_node == "," and nodes_stack[-3].type_node == "expr"):
                                param_list = ['goto', 81]

                        if(param_list!=None and param_list[0]=='goto' and param_list[1]==32):
                            if(listaNodos_analize[index].type_node == ")" and nodes_stack[-1].type_node == "expr" and nodes_stack[-2].type_node == "(" and nodes_stack[-3].type_node != "if"):
                                param_list = ['goto', 59]
                        ...
                        if(param_list!=None and param_list[0]=='goto' and param_list[1]==32):
                            if(nodes_stack[-1].type_node == "expr" and nodes_stack[-2].type_node == "bin_op" and nodes_stack[-3].type_node == "expr"):
                                param_list = ['goto', 69]
                        ...
                        if(param_list!=None and param_list[0]=='goto' and param_list[1]==32):
                            if(listaNodos_analize[index].type_node == "," and nodes_stack[-1].type_node == "expr" and nodes_stack[-2].type_node == "assign_op"):
                                param_list = ['goto', 65]

                elif(param_list[0]=='accept'):
                    print("accept :)")
            else:
                if(index<len(listaNodos_analize)):
                    listaErrores.append("Parse error: unexpected token " + listaNodos_analize[index].type_node + " at line " + str(listaNodos_analize[index].objeto.line))
                else:
                    listaErrores.append("Parse error: unexpected token " + listaNodos_analize[index-1].type_node + " at line " + str(listaNodos_analize[index-1].objeto.line))
                break
    ...
    def accepts(self, token_list):
        ...
        current_token = token_list[index].tipoS.name
        ...
        while index<=len(token_list):
            ...
            if (param_list != None):
                if(param_list[0]=='shift'):
                    ...
                    print(param_list[1])
                    node = Nodo.Nodo(list(self.grammar[param_list[1]-1].keys())[0], list(self.grammar[param_list[1]-1].values())[0])
                ...
                if(index<len(token_list)):
                    print("unexpected token",token_list[index].tipoS.name,"at line",token_list[index].line)
                else:
                    print("unexpected token",token_list[index-1].tipoS.name,"at line",token_list[index-1].line)
                break
